...
function draw() {
	...
	info.textContent = `Array Length: ${ set.buckets.length } - Load Factor: ${ percent(set.loadFactor) } (thresholds: ${ percent(HashSet.loadThreshold.min) }-${percent(HashSet.loadThreshold.max)}) - Size: ${ set.size }`;
	rainbowOffset += 1;
}
